...
if (class_exists('ZC_DMT_Indicators')) {
    ...
    wp_die(__('Indicator class not found.', 'zc-dmt'));
}

// Handle form submissions
if (isset($_POST['submit']) && isset($_POST['zc_dmt_edit_indicator_nonce'])) {
    // Verify nonce
    if (!wp_verify_nonce($_POST['zc_dmt_edit_indicator_nonce'], 'zc_dmt_edit_indicator')) {
        wp_die(__('Security check failed', 'zc-dmt'));
    }
    
    // Process form data
    $name = sanitize_text_field($_POST['indicator_name']);
    $slug = sanitize_key($_POST['indicator_slug']);
    $description = sanitize_textarea_field($_POST['indicator_description']);
    ...
    // Update indicator
    $result = $indicators->update_indicator($indicator_id, $indicator_data);
    ...
    if (is_wp_error($result)) {
        ...
        wp_redirect(admin_url('admin.php?page=zc-dmt-indicators&updated=1'));
        exit;
    }
}

// Set current values
$current_name = $indicator->name;
$current_slug = $indicator->slug;
$current_description = $indicator->description;
$current_source_type = $indicator->source;
$current_frequency = $indicator->frequency;
$current_units = $indicator->unit;
$current_is_active = $indicator->is_active;
$current_source_config = isset($indicator->source_config) ? $indicator->source_config : array();
...
<div class="wrap zc-dmt-edit-indicator">
    ...
    <form method="post" action="">
        <?php wp_nonce_field('zc_dmt_edit_indicator', 'zc_dmt_edit_indicator_nonce'); ?>
        <input type="hidden" name="indicator_id" value="<?php echo esc_attr($indicator_id); ?>">
        ...
        <div class="zc-indicator-form">
                            ...
                            <select name="source_type" id="source_type" required>
                                ...
                                <?php foreach ($source_types as $key => $source) : ?>
                                    <option value="<?php echo esc_attr($key); ?>" 
                                            <?php selected($current_source_type, $key); ?>>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="frequency" id="frequency">
                                ...
                                <option value="monthly" <?php selected($current_frequency, 'monthly'); ?>>
                                    <?php _e('Monthly', 'zc-dmt'); ?>
                                </option>
                                <option value="quarterly" <?php selected($current_frequency, 'quarterly'); ?>>
                                    <?php _e('Quarterly', 'zc-dmt'); ?>
                                </option>
                                <option value="yearly" <?php selected($current_frequency, 'yearly'); ?>>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="units" id="units" 
                                   value="<?php echo esc_attr($current_units); ?>" class="regular-text">
            ...
            <div class="zc-form-section">
                ...
                <div class="zc-source-config" id="source-config-container">
                    <?php if (!empty($current_source_type) && isset($source_types[$current_source_type])) : ?>
                        <?php 
                        $config_fields = isset($source_types[$current_source_type]['config_fields']) ? 
                                         $source_types[$current_source_type]['config_fields'] : array();
        ...
        <?php submit_button(__('Update Indicator', 'zc-dmt')); ?>
    </form>
</div>
