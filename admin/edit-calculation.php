...
if (isset($_POST['submit']) && isset($_POST['zc_dmt_edit_calculation_nonce'])) {
    // Verify nonce
    if (!wp_verify_nonce($_POST['zc_dmt_edit_calculation_nonce'], 'zc_dmt_edit_calculation')) {
        wp_die(__('Security check failed', 'zc-dmt'));
    }
    
    // Process form data
    $name = sanitize_text_field($_POST['calculation_name']);
    $formula = sanitize_textarea_field($_POST['calculation_formula']);
    $output_type = sanitize_key($_POST['output_type']);
    ...
    // Update calculation
    $result = $calculations->update_calculation($calculation_id, $calculation_data);
    ...
    if (is_wp_error($result)) {
        ...
        wp_redirect(admin_url('admin.php?page=zc-dmt-calculations&updated=1'));
        exit;
    }
}

// Set current values
$current_name = $calculation->name;
$current_formula = $calculation->formula;
$current_output_type = $calculation->output_type;
...
<div class="wrap zc-dmt-edit-calculation">
    ...
    <form method="post" action="">
        <?php wp_nonce_field('zc_dmt_edit_calculation', 'zc_dmt_edit_calculation_nonce'); ?>
        <input type="hidden" name="calculation_id" value="<?php echo esc_attr($calculation_id); ?>">
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="output_type" id="output_type">
                                <option value="single" <?php selected($current_output_type, 'single'); ?>>
                                    <?php _e('Single Value', 'zc-dmt'); ?>
                                </option>
                                <option value="series" <?php selected($current_output_type, 'series'); ?>>
                ...
                <div class="zc-formula-builder">
                    ...
                    <div class="zc-formula-panel">
                        ...
                        <textarea name="calculation_formula" id="calculation_formula" 
                                  class="large-text code" rows="10"><?php echo esc_textarea($current_formula); ?></textarea>
        ...
        <?php submit_button(__('Update Calculation', 'zc-dmt')); ?>
    </form>
</div>
